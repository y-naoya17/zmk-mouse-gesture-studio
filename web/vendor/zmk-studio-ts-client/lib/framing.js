    ...
    return {
        transform: (chunk, controller) => {
            if (chunk instanceof Uint8Array) {
                ...
                return controller.error('Only Uint8Array chunks are able to be handled');
            }
        },
    };
}
...
(function (DecodeState) {
    DecodeState[DecodeState["IDLE"] = 0] = "IDLE";
    DecodeState[DecodeState["AWAITING_DATA"] = 1] = "AWAITING_DATA";
    DecodeState[DecodeState["ESCAPED"] = 2] = "ESCAPED";
})(DecodeState || (DecodeState = {}));
export function get_decoder() {
    let state = DecodeState.IDLE;
    let data = [];
    let process = (b, controller) => {
        switch (state) {
